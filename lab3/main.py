...
class SettingsDialog(QDialog):
    ...
    def change_cloud(self):
        ...
        if self.cloud.num_drops > self.num_drops_input.value():
            self.cloud.drops = self.cloud.drops[: self.num_drops_input.value()]
            if self.cloud.create_timer.isActive():
                self.cloud.create_timer.stop()
        else:
            self.cloud.left_drops += (
                self.num_drops_input.value() - self.cloud.num_drops
            )
            if not self.cloud.create_timer.isActive():
                self.cloud.create_timer.start(10)
